        ...
        {userDetails.address.map((e) => (
          <StyledAddress key={e.id}>
            {toggleEdit.state === true && toggleEdit.id === e.id ? (
              newFunction(setUpdate, update, setUserDetails, userDetails, toggleEdit, e, setToggleEdit)
            ) : (
              <div>
                <h4>{e.name}</h4>
                <p>{e.mobile}</p>
...
export default User
function newFunction(setUpdate, update, setUserDetails, userDetails, toggleEdit, e, setToggleEdit) {
  return <StyledAddressForm
    onSubmit={(e) => {
      e.preventDefault()
      setUpdate(update)
      console.log(update)
  ...
  >
    ...
    <button>save</button>
    <button
      onClick={() => setToggleEdit({ id: null, state: false })}
    >
      close
    </button>
  </StyledAddressForm>
